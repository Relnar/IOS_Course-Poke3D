...
//  Poke3D
//
//  Created by Pierre-Luc Bruyere on 2018-11-06.
...
{
  mutating func minimize(min vector: SCNVector3)
  {
    self = SCNVector3(Float.minimum(self.x, vector.x),
                      Float.minimum(self.y, vector.y),
                      Float.minimum(self.z, vector.z))
  }

  mutating func maximize(max vector: SCNVector3)
  {
    self = SCNVector3(Float.maximum(self.x, vector.x),
                      Float.maximum(self.y, vector.y),
                      Float.maximum(self.z, vector.z))
  }
